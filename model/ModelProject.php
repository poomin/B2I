...
class ModelProject extends  _DBPDO
{


    public $DB = "b2i_project";
    public $FK_PROJECTSETUP = "b2i_projectsetup";
    public $FK_PROJECT_MEMBER = "b2i_project_member";
    public $FK_PROJECT_PHASE = "b2i_project_phase";
    public $FK_PROJECT_CONFIRM = "b2i_project_confirm";
     ...
     * user-project.php
     */
    function selectForUserProject($user_id,$project_setup_id){
        //set parameter
        $this_db = $this->DB;
        ...
        //connect DB
        $this->connect();
        $sql = "select p.* from ( select project_id from $this_p_member where user_id=:user_id ) as member 
                left join $this_db as p on p.id = member.project_id
                WHERE p.projectsetup_id =:ps_id ";
        $params= array(':user_id'=> $user_id , ':ps_id'=>$project_setup_id);
        $result = $this->queryAll($sql,$params);
    ...
    function getProjectLastStatus($project_id){
        ...
        $this_db = $this->DB;
        $this_p_member = $this->FK_PROJECT_MEMBER;
        $this_psetup = $this->FK_PROJECTSETUP;
        $this_p_phase = $this->FK_PROJECT_PHASE;
        $this_p_confirm = $this->FK_PROJECT_CONFIRM;


        //connect DB
        $this->connect();

        $sql = "select p_setup.* from $this_db as pro 
        left join $this_psetup as p_setup on pro.projectsetup_id = p_setup.id
        where pro.id=:project_id";
        $params= array(':project_id'=> $project_id);
        $projectSetup = $this->query($sql,$params);

        $sql = "select * from $this_p_phase where project_id=:project_id";
        $params= array(':project_id'=> $project_id);
        ...
        $sql = "select * from $this_p_confirm where project_id=:project_id";
        $params= array(':project_id'=> $project_id);
